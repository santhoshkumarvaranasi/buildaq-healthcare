  ...
  template: `
    <div class="patients-page">
      <div class="hero mat-elevation-z2">
        <div class="hero-text">
          <div class="hero-icon">
            <svg viewBox="0 0 24 24" class="icon"><path d="M4 7h12a3 3 0 0 1 3 3v7h-2v-3H4v3H2V7h2Zm0 5h13v-2a1 1 0 0 0-1-1H5a1 1 0 0 0-1 1v2Z" fill="currentColor"/></svg>
          </div>
          <div>
            <p class="eyebrow">Healthcare Management</p>
            <h1>Bed Management</h1>
            <p class="subtitle">Track occupancy, discharges, and isolation flags.</p>
          </div>
        </div>
      ...
      <mat-card class="filters-card mat-elevation-z2">
        <div class="filter-grid compact">
          <mat-form-field appearance="fill" class="full-width search-field">
            ...
            <input matInput [(ngModel)]="search" (input)="applyFilters()" placeholder="bed, unit, patient" />
          ...
          <div class="chip-group">
            ...
            <mat-chip-listbox class="status-chips" [value]="statusFilter" [multiple]="false" (valueChange)="onStatusChange($event)">
              ...
              <mat-chip-option value="Isolation">Isolation</mat-chip-option>
            </mat-chip-listbox>
          </div>
      ...
      <mat-card class="table-card mat-elevation-z2">
        <div class="table-head">
          <div>
            <mat-card-title>Bed list</mat-card-title>
            <mat-card-subtitle>Occupancy by unit</mat-card-subtitle>
          </div>
        ...
        <div class="table-wrap desktop-only">
          <table mat-table [dataSource]="dataSource" matSort class="hc-table">
            <ng-container matColumnDef="bed">
              ...
              <td mat-cell *matCellDef="let row">{{ row.bed }}</td>
            </ng-container>
            ...
            <tr class="empty-state" *matNoDataRow>
              <td [attr.colspan]="displayedColumns.length">
                <div class="fee-empty">No beds match your filters.</div>
              </td>
            </tr>
          </table>
      ...
      <mat-card class="form-card mat-elevation-z2">
        <mat-card-title>{{ editIndex === null ? 'Add Bed' : 'Edit Bed' }}</mat-card-title>
        <div class="form-grid">
          <mat-form-field appearance="fill">
            <mat-label>Bed</mat-label>
            <input matInput [(ngModel)]="form.bed" />
          </mat-form-field>
          <mat-form-field appearance="fill">
            <mat-label>Unit</mat-label>
...
export class HcBedsComponent implements AfterViewInit {
  rows: BedRow[] = [
    { bed: 'ICU-101', unit: 'ICU', status: 'Occupied', patient: 'Amira Patel', notes: 'Vent wean' },
    { bed: 'ICU-102', unit: 'ICU', status: 'Isolation', patient: 'N/A', notes: 'Clean, ready' },
    { bed: 'MS-201', unit: 'Med/Surg', status: 'Occupied', patient: 'Lena Chen', notes: 'POD2' },
    { bed: 'MS-202', unit: 'Med/Surg', status: 'Available', patient: '', notes: 'Cleaned' }
  ];
  dataSource = new MatTableDataSource<BedRow>(this.rows);
  displayedColumns = ['bed','unit','status','patient','notes','actions'];
  metricCards = [
    { label: 'Total', value: 0, meta: 'All beds' },
    { label: 'Occupied', value: 0, meta: 'In use' },
    { label: 'Available', value: 0, meta: 'Ready' }
  ];

  units = ['ICU','Med/Surg','ED','Stepdown'];
  search = '';
  unitFilter = '';
  statusFilter = 'all';

  editIndex: number | null = null;
  form: BedRow = { bed: '', unit: 'ICU', status: 'Available', patient: '', notes: '' };
  ...
  applyFilters(): void {
    ...
    const filtered = this.rows.filter(r => {
      const matchesTerm = !term || r.bed.toLowerCase().includes(term) || r.unit.toLowerCase().includes(term) || r.patient.toLowerCase().includes(term);
  ...
  refreshMetrics(): void {
    ...
    this.metricCards = [
      { label: 'Total', value: data.length, meta: 'All beds' },
      { label: 'Occupied', value: occupied, meta: 'In use' },
      { label: 'Available', value: available, meta: 'Ready' }
    ];
  }

  startAdd(): void {
    this.editIndex = null;
    this.form = { bed: '', unit: 'ICU', status: 'Available', patient: '', notes: '' };
  }
  ...
  save(): void {
    if (this.editIndex === null) {
      ...
      const idx = this.rows.findIndex(r => r.bed === target.bed && r.unit === target.unit);
  ...
  delete(index: number): void {
    ...
    this.rows = this.rows.filter(r => !(r.bed === target.bed && r.unit === target.unit));
    this.dataSource.data = [...this.rows];
    this.applyFilters();
  }
}
