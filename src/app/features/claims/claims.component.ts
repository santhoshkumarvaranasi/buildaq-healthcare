  ...
  template: `
    <div class="patients-page">
      <div class="hero mat-elevation-z2">
        <div class="hero-text">
          <div class="hero-icon">
            <svg viewBox="0 0 24 24" class="icon"><path d="M4 5h16v14H4V5Zm2 2v10h12V7H6Zm7 1h3v2h-3V8Zm-6 4h10v2H7v-2Z" fill="currentColor"/></svg>
          </div>
          <div>
            <p class="eyebrow">Healthcare Management</p>
            <h1>Claims</h1>
            <p class="subtitle">Pipeline by payer with mock data.</p>
          </div>
        </div>
      ...
      <mat-card class="filters-card mat-elevation-z2">
        <div class="filter-grid compact">
          <mat-form-field appearance="fill" class="full-width search-field">
            ...
            <input matInput [(ngModel)]="search" (input)="applyFilters()" placeholder="payer, status" />
      ...
      <mat-card class="table-card mat-elevation-z2">
        <div class="table-head">
          <div>
            <mat-card-title>Claim pipeline</mat-card-title>
            <mat-card-subtitle>By payer and status</mat-card-subtitle>
          </div>
        ...
        <div class="table-wrap desktop-only">
          <table mat-table [dataSource]="dataSource" matSort class="hc-table">
            ...
            <ng-container matColumnDef="denials">
              <th mat-header-cell *matHeaderCellDef mat-sort-header>Denials</th>
              <td mat-cell *matCellDef="let row">{{ row.denials }}</td>
            </ng-container>
            ...
            <tr class="empty-state" *matNoDataRow>
              <td [attr.colspan]="displayedColumns.length">
                <div class="fee-empty">No claims match your filters.</div>
              </td>
            </tr>
          </table>
      ...
      <mat-card class="form-card mat-elevation-z2">
        <mat-card-title>{{ editIndex === null ? 'Add Claim' : 'Edit Claim' }}</mat-card-title>
        <div class="form-grid">
          <mat-form-field appearance="fill">
            <mat-label>Payer</mat-label>
            <input matInput [(ngModel)]="form.payer" />
          </mat-form-field>
          <mat-form-field appearance="fill">
          ...
          <mat-form-field appearance="fill">
            ...
            <input matInput type="number" [(ngModel)]="form.aging" />
          </mat-form-field>
          <mat-form-field appearance="fill">
            <mat-label>Denials</mat-label>
            <input matInput type="number" [(ngModel)]="form.denials" />
          </mat-form-field>
        </div>
...
export class HcClaimsComponent implements AfterViewInit {
  rows: ClaimRow[] = [
  ...
  dataSource = new MatTableDataSource<ClaimRow>(this.rows);
  displayedColumns = ['payer','amount','status','aging','denials','actions'];
  metricCards = [
    { label: 'Total', value: 0, meta: 'All claims' },
    { label: 'Submitted/Pending', value: 0, meta: 'Open' },
    { label: 'Denied', value: 0, meta: 'Needs work' }
  ];

  search = '';
  statusFilter = '';
  editIndex: number | null = null;
  form: ClaimRow = { payer: '', amount: 0, status: 'Submitted', aging: 0, denials: 0 };
  ...
  applyFilters(): void {
    ...
    const filtered = this.rows.filter(r => {
      const matchesTerm = !term || r.payer.toLowerCase().includes(term) || r.status.toLowerCase().includes(term);
  ...
  refreshMetrics(): void {
    ...
    const open = data.filter(r => r.status === 'Submitted' || r.status === 'Pending').length;
    const denied = data.filter(r => r.status === 'Denied').length;
    this.metricCards = [
      { label: 'Total', value: data.length, meta: 'All claims' },
      { label: 'Submitted/Pending', value: open, meta: 'Open' },
      { label: 'Denied', value: denied, meta: 'Needs work' }
    ];
  }

  startAdd(): void {
    this.editIndex = null;
    this.form = { payer: '', amount: 0, status: 'Submitted', aging: 0, denials: 0 };
  }
  ...
  save(): void {
    if (this.editIndex === null) {
      ...
      const idx = this.rows.findIndex(r => r.payer === target.payer && r.amount === target.amount && r.aging === target.aging);
  ...
  delete(index: number): void {
    ...
    this.rows = this.rows.filter(r => !(r.payer === target.payer && r.amount === target.amount && r.aging === target.aging));
    this.dataSource.data = [...this.rows];
    this.applyFilters();
  }
}
